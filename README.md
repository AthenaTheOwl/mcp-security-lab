...
The scanner is graded against 25 publicly documented MCP configurations from `modelcontextprotocol/servers`, `modelcontextprotocol/servers-archived`, `geelen/mcp-remote`, and `github/github-mcp-server`. The corpus keeps its provenance in `eval/manifest.json`; hand-labeled ground truth lives in `eval/labels.json`.

The current pinned scorecard is 100% precision and 100% recall across every supported category with positive examples:

| Category | Rule | Precision | Recall | FP | FN |
| --- | --- | ---: | ---: | ---: | ---: |
| command execution | `STDIO-COMMAND` | 100% | 100% | 0 | 0 |
| broad filesystem | `BROAD-ACCESS` | 100% | 100% | 0 | 0 |
| unauthenticated remote | `REMOTE-NO-AUTH` | 100% | 100% | 0 | 0 |
| prompt injection | `INJECTION-CORPUS` | n/a | n/a | 0 | 0 |
| **overall (micro)** | - | **100%** | **100%** | **0** | **0** |

Two earlier misses now have regression coverage: a Docker `--mount type=bind,src=/Users/...` whole-home mount, and a remote SSE URL passed through `npx mcp-remote`. The scorecard is regenerated by `python scripts/generate_eval_scorecard.py` and pinned in CI.

That result has a defined boundary. The public config corpus has no positive prompt-injection examples because desktop config files carry commands, arguments, and environment values rather than runtime tool descriptions. Static config analysis also cannot inspect package behavior, prove that declared auth works, or reason about capabilities hidden behind an innocuous tool name. Those remain separate runtime and judgment problems.
