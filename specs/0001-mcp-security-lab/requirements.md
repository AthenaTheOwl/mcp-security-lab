...
| R-MCPSEC-MCPSURF-003 | Parameterize the MCP surface drift gate over `config/mcp_server_registry.yaml`; gate every enabled entry by default and support `--server-id <id>` for single-server runs. | owner_role:implementation_agent |
| R-MCPSEC-MCPSURF-004 | Emit one diff report per gated server at `reports/mcp-surface-diff.<server-id>.json`; CI invokes the gate with `--all` so adding a registry entry enrolls the new server without workflow edits. | owner_role:implementation_agent |
| R-MCPSEC-MCPSURF-005 | Validate the registry on load: reject malformed YAML, missing required fields, duplicate ids, and non-boolean `enabled`; skip entries with `enabled: false` from the default-all gate run. | owner_role:review_agent |
| R-MCPSEC-010 | Mirror athena-site's amended `decision.schema.json`, `dream-output.schema.json`, and `run.schema.json` under `ops/schemas-cache/` so the offline-CI cache carries the four optional systems-thinking fields (`systems_map`, `transferable_principle`, `falsification_test`, `adoption_ladder`). | owner_role:review_agent |
| R-MCPSEC-011 | Name the systems-thinking discipline in `.agents/AGENTS.md` so agents landing changes in this repo know the four-field contract for new DECs, dream candidates, and Run records. | owner_role:planning_agent |
| R-MCPSEC-012 | Extend `scripts/validate_decisions.py` to emit a stderr warning (not failure) when an approved DEC misses any of the four systems-thinking fields. Exit code stays 0 during the 30-day bootstrap window. | owner_role:implementation_agent |
| R-MCPSEC-013 | Retrofit the three most-recent DECs (DEC-MCPSEC-006/007/008) with substantive `systems_map`, `transferable_principle`, `falsification_test`, and `adoption_ladder` fields to demonstrate the discipline. | owner_role:implementation_agent |
| R-MCPSEC-EVAL-001 | Score the scanner against a curated corpus of real, publicly documented MCP configs (`eval/`, with a provenance manifest and hand-labelled ground truth) and commit a deterministic precision/recall scorecard to `reports/eval-scorecard.{json,md}`; a test regenerates it over the committed corpus and fails when the numbers drift. | owner_role:review_agent |
